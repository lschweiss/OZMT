...
output_map () {
    ...
    if [ "$wide" == 'true' ]; then    
        printf '%3s | %-22s | %-15s | %-9s | %-15s | %-4s | %-8s | %-10s | %-9s | %-5s | %-5s | %-5s | %-5s | %-5s | %-16s | %-16s | %-16s | %-16s\n' \
            "Bay" "OS Name" "Serial" "Vendor" "Model" "FW" "Status" "Pool" "vdev" "sd" "12345" "SoftE" "HardE" "TranE" "Slot Status" "wwn" "Addr 1" "Addr 2"
    else
    ...
    while [ $bay -lt $bays ]; do
        ...
        softE="${disk["${disk_wwn}_softerror"]}"
        hardE="${disk["${disk_wwn}_harderror"]}"
        transE="${disk["${disk_wwn}_transerror"]}"

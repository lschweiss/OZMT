...
if [ -f $os/interface-functions.sh ]; then
    ...
    error "enable-network.sh: unsupported OS: $os"
    exit 1
fi
...
if [ "${interface_definition["$interface"]}" == '' ]; then
    if physical_if $interface; then
        activate_if $interface $override_persistent
    else
        error "enable-network.sh: Unknown interface $interface $override_persistent"
        exit 1
    fi
else
    activate_if $interface $override_persistent

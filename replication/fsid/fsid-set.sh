...
if [ "$fsid" != "" ] && [ "$mem" != "" ]; then
    ...
    if [ $result -eq 0 ]; then
        # Remove collected fsid
        rm -f ${MYTMP}/$folder_file

        # Push change back to production zfs user parameter
        source_pool=`cat /${pool}/zfs_tools/var/replication/source/${dataset} | $CUT -d ':' -f1`
        source_folder="${source_pool}${folder:${#pool}}"
        $SSH ${source_pool} zfs set ${zfs_fsid_property}:${pool}=${new_fsid} $source_folder
    fi

else
    # Something went wrong.  Delete the bad data file and exit.
    warning "FSID or Mem address not collected properly for $folder"
